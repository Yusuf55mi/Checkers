...
import { Params } from "alice-checkers-client-ts/checkers.checkers/types"
import { StoredGame } from "alice-checkers-client-ts/checkers.checkers/types"
import { SystemInfo } from "alice-checkers-client-ts/checkers.checkers/types"
...
export default {
	...
	actions: {
		...
		async sendMsgPlayMove({ rootGetters }, { value, fee = {amount: [], gas: "200000"}, memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const fullFee = Array.isArray(fee)  ? {amount: fee, gas: "200000"} :fee;
				const result = await client.CheckersCheckers.tx.sendMsgPlayMove({ value, fee: fullFee, memo })
				return result
			} catch (e) {
		...
		async sendMsgCreateGame({ rootGetters }, { value, fee = {amount: [], gas: "200000"}, memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const fullFee = Array.isArray(fee)  ? {amount: fee, gas: "200000"} :fee;
				const result = await client.CheckersCheckers.tx.sendMsgCreateGame({ value, fee: fullFee, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
